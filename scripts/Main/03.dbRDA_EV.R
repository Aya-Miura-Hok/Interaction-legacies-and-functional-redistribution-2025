...
df_env <- as(sample_data(fpomsize), "data.frame") %>%
  ...
  select(Sample, Type, Treatment, d15N, d13C, HIX, BIX, DON)

# Merge OTU and environmental data
df_combined <- inner_join(df_phylum_filtered, df_env, by = "Sample")
df_phylum <- df_combined %>% select(Sample, all_of(selected_phyla)) %>% column_to_rownames(var = "Sample")
df_envs <- df_combined %>% select(Sample, d13C, HIX, BIX, DON) %>% column_to_rownames(var = "Sample")
...
sample_scores <- left_join(sample_scores, df_combined %>% select(Sample, Type, Treatment), by = "Sample")
...
p <- ggplot(sample_scores, aes(x = CAP1, y = CAP2, color = Type, shape = Treatment)) +
  ...
  geom_segment(data = bp_scores, aes(x = 0, y = 0, xend = CAP1 * 1.2, yend = CAP2 * 1.2),
               inherit.aes = FALSE, arrow = arrow(length = unit(0.15, "cm")), color = "black", linewidth = 0.7) +
  geom_text_repel(data = bp_scores, aes(x = CAP1 * 1.4, y = CAP2 * 1.4, label = var),
                  inherit.aes = FALSE, color = "black", size = 5) +
  geom_segment(data = species_scores, aes(x = 0, y = 0, xend = CAP1 * 1.2, yend = CAP2 * 1.2, size = length),
               inherit.aes = FALSE, arrow = arrow(length = unit(0.1, "cm")), color = "#F8766D", linetype = "dashed") +
  geom_text_repel(data = species_scores, aes(x = CAP1 * 1.5, y = CAP2 * 1.5, label = phylum),
                  inherit.aes = FALSE, color = "#F8766D", size = 5,
  ...
  labs(
    title = "dbRDA: Phylum vs Environmental Vectors (Bray-Curtis)",
    x = "CAP1", y = "CAP2",
    color = "Type",
    shape = "Treatment",
    size = "Impact (R²)"
