...
run_glm_analysis <- function(df_corr, response_var) {
  ...
  ft <- flextable(df_result) %>%
    ...
    border_remove() %>%
    border(part = "header", border.top = fp_border(color = "black", width = 1), border.bottom = fp_border(color = "black", width = 1)) %>%
    border(i = nrow(df_result), part = "body", border.bottom = fp_border(color = "black", width = 1)) %>%
    autofit()
