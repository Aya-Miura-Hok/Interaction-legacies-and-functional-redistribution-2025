...
df_phylum_filtered <- select(otu_table_long, Sample, all_of(selected_phyla))
...
df_ko_filtered <- df_ko_norm %>% select(Sample, all_of(valid_kos))
df_pathabun_filtered <- df_pathabun_norm %>% select(Sample, all_of(valid_pathways))
...
df_phylum <- df_combined_all %>% select(Sample, all_of(selected_phyla)) %>% column_to_rownames(var = "Sample")
df_ko_pwy <- df_combined_all %>% select(Sample, starts_with("K"), starts_with("PWY")) %>% column_to_rownames(var = "Sample")
...
# Join sample info
df_env <- as(sample_data(fpomsize), "data.frame") %>% rownames_to_column("Sample") %>% select(Sample, Type, d13C, HIX, BIX)
df_combined_all <- left_join(df_combined_all, df_env, by = "Sample")
sample_scores <- left_join(sample_scores, df_combined_all %>% select(Sample, Type), by = "Sample")

# Define colors
type_colors <- c("alder_l" = "#D73027", "alder_s" = "#FC8D59", "algae_l" = "#1A9850", "algae_s" = "#91CF60", "feces_l" = "#4575B4", "feces_s" = "#74ADD1")

# Plot
p <- ggplot() +
  geom_point(data = sample_scores, aes(x = CAP1, y = CAP2, fill = Type), shape = 21, size = 3, alpha = 0.8, color = "black") +
  geom_segment(data = species_scores, aes(x = 0, y = 0, xend = CAP1 * 1.5, yend = CAP2 * 1.5), arrow = arrow(length = unit(0.1, "cm")), color = "#F8766D", linetype = "dashed") +
  geom_text_repel(data = species_scores, aes(x = CAP1 * 1.7, y = CAP2 * 1.7, label = KO_PWY), color = "#F8766D", size = 5) +
  geom_segment(data = bp_scores, aes(x = 0, y = 0, xend = CAP1 * 1.2, yend = CAP2 * 1.2), arrow = arrow(length = unit(0.2, "cm")), color = "black", linetype = "solid") +
  geom_text_repel(data = bp_scores, aes(x = CAP1 * 1.3, y = CAP2 * 1.3, label = rownames(bp_scores)), color = "black", size = 5) +
  scale_fill_manual(values = type_colors) +
  theme_minimal(base_size = 16) +
  theme(axis.title = element_text(size = 18, face = "bold"), axis.text = element_text(size = 16), legend.title = element_text(size = 16, face = "bold"), legend.text = element_text(size = 14), plot.title = element_text(size = 20, face = "bold", hjust = 0.5)) +
  labs(title = "dbRDA: Phylum vs KO/PWY (p < 0.05)", x = "CAP1", y = "CAP2", fill = "Type")
