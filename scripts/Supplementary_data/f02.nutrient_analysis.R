...
analyze_fpom_auto <- function(ps_type, type_label) {
  ...
  df$NH4 <- as.numeric(as.character(df$NH4))
  df$NO3 <- as.numeric(as.character(df$NO3))
  df$PO4 <- as.numeric(as.character(df$PO4))
  ...
  for (var in c("DOC", "DON", "NH4", "NO3", "PO4")) {
    cat("\n--- Variable:", var, "---\n")
    response <- df[[var]]
  ...
  df_long <- melt(df, id.vars = c("Treatment", "Size"),
                  measure.vars = c("DOC", "DON", "NH4", "NO3", "PO4"),
                  variable.name = "Variable", value.name = "Value")

  df_long$Treatment <- factor(df_long$Treatment, levels = c("C", "I", "M"))
  df_long$Variable <- factor(df_long$Variable, levels = c("DOC", "DON", "NH4", "NO3", "PO4"))
